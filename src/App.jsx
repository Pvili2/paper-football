...
const PaperSoccerGame = () => {
  ...
  let currentSizeIndex = fieldSizes.findIndex(size => size.rows === rows && size.cols === cols);
  ...
  const initializeLines = () => {
    ...
    for (let i = 0; i < rows; i++) {
      if (i < Math.floor(rows / 3) || i > Math.floor(2 * rows / 3)) {
        newLines.push({ start: { x: 0, y: i }, end: { x: 0, y: i + 1 }, player: 0 });
        newLines.push({ start: { x: cols, y: i }, end: { x: cols, y: i + 1 }, player: 0 });
      }
    }
    for (let j = 0; j < cols; j++) {
      newLines.push({ start: { x: j, y: 0 }, end: { x: j + 1, y: 0 }, player: 0 });
      newLines.push({ start: { x: j, y: rows }, end: { x: j + 1, y: rows }, player: 0 });
    }
    setLines(newLines);
  };
  ...
  const makeMove = (x, y) => {
    if (isValidMove(x, y)) {
      const newLines = [...lines, { start: ballPosition, end: { x, y }, player: currentPlayer }];
      setLines(newLines);
      ...
      if (checkWin(x, y)) {
        ...
        triggerConfetti();
        if (currentPlayer === 2 && x === 0 && y >= Math.floor(rows / 3) && y <= Math.floor(2 * rows / 3)) {
          setScores(prev => ({ ...prev, player2: prev.player2 + 1 }));
          return true;
        }
        if (currentPlayer === 1 && x === cols && y >= Math.floor(rows / 3) && y <= Math.floor(2 * rows / 3)) {
  ...
  const getBestMove = (availableMoves) => {
    // Improved strategy: Consider both distance to goal and available moves after this move
    return availableMoves.reduce((best, move) => {
      const distanceToGoal = Math.abs(move.x - (currentPlayer === 1 ? cols : 0));
      const movesAfter = getAvailableMoves().length;
      const score = distanceToGoal * 2 + movesAfter; // Prioritize moves towards goal but also consider future moves
  ...
  const renderGameModeSelection = () => (
    <div style={{
      ...
      <h2 style={{ color: '#ffffff', textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>Select Game Mode</h2>
      <div style={{ display: 'flex', gap: '10px' }}>
        <button
          ...
          style={{
            ...
            fontWeight: 'bold',
            color: '#ffffff',
            backgroundColor: gameMode === 'player' ? '#3498db' : 'rgba(52, 152, 219, 0.7)',
            border: 'none',
            borderRadius: '5px',
            cursor: 'pointer',
        ...
        <button
          ...
          style={{
            ...
            fontWeight: 'bold',
            color: '#ffffff',
            backgroundColor: gameMode === 'ai' ? '#2ecc71' : 'rgba(46, 204, 113, 0.7)',
            border: 'none',
            borderRadius: '5px',
            cursor: 'pointer',
            transition: 'all 0.3s ease',
      ...
      {gameMode === 'ai' && (
        <div>
          <h3 style={{ color: '#ffffff', textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>AI Difficulty</h3>
          <select
            value={aiDifficulty}
  ...
  const isValidMove = (x, y) => {
    ...
    if (Math.abs(x - ballPosition.x) > 1 || Math.abs(y - ballPosition.y) > 1) return false;
    if (x === ballPosition.x && y === ballPosition.y) return false;

    // Check if the line already exists
    const lineExists = lines.some(
      line =>
        (line.start.x === ballPosition.x && line.start.y === ballPosition.y &&
          line.end.x === x && line.end.y === y) ||
        (line.end.x === ballPosition.x && line.end.y === ballPosition.y &&
    ...
    return !lineExists && (!isEdgeMove || (
      (x === 0 || x === cols) && y > Math.floor(rows / 3) && y < Math.floor(2 * rows / 3)
    ));
  };

  const checkWin = (x, y) => {
    // Bal oldali kapu (2. játékos)
    if (currentPlayer === 2 && x === 0 && y >= Math.floor(rows / 3) && y <= Math.floor(2 * rows / 3)) return true;
    // Jobb oldali kapu (1. játékos)
    if (currentPlayer === 1 && x === cols && y >= Math.floor(rows / 3) && y <= Math.floor(2 * rows / 3)) return true;

    // Kapu alján lévő vonal érintése
    if (currentPlayer === 1 && x === cols - 1 && y === Math.floor(2 * rows / 3) + 1) return true;
    if (currentPlayer === 2 && x === 1 && y === Math.floor(2 * rows / 3) + 1) return true;
        ...
        points.push(
          <motion.div
            ...
            style={{
              ...
              height: `${pointSize}px`,
              borderRadius: '50%',
              background: x === ballPosition.x && y === ballPosition.y ? '#FFD700' :
                visitedPoints[`${x}-${y}`] ? '#FF69B4' : 'rgba(255, 255, 255, 0.5)',
              cursor: 'pointer',
              zIndex: 2,
  ...
  const renderLines = () => {
    return lines.map((line, index) => (
      <motion.line
        ...
        stroke={line.player === 1 ? '#3498db' : line.player === 2 ? '#2ecc71' : 'rgba(255, 255, 255, 0.3)'}
        strokeWidth="3"
    ...
    return (
      <>
        ...
        <g>
          <path
            d={`
              ...
              L ${padding + goalDepth} ${padding + Math.floor(2 * rows / 3) * cellSize + goalDepth}
              L ${padding + goalDepth} ${padding + Math.floor(rows / 3) * cellSize - goalDepth}
              Z
            `}
            fill="#2c3e50"
            stroke="#3498db"
        ...
        <g>
          <path
            d={`
              M ${padding + cols * cellSize} ${padding + Math.floor(rows / 3) * cellSize}
              L ${padding + cols * cellSize} ${padding + Math.floor(2 * rows / 3) * cellSize}
              L ${padding + cols * cellSize - goalDepth} ${padding + Math.floor(2 * rows / 3) * cellSize + goalDepth}
              L ${padding + cols * cellSize - goalDepth} ${padding + Math.floor(rows / 3) * cellSize - goalDepth}
              Z
            `}
            fill="#2c3e50"
            stroke="#2ecc71"
  ...
  const renderResizeButtons = () => {
    const prevIndex = (currentSizeIndex - 1 + fieldSizes.length) % fieldSizes.length;
    const nextIndex = (currentSizeIndex + 1) % fieldSizes.length;
    const previewSize = 300; // Növeltük az előnézet méretét

    return (
      <>
        <div style={{ position: 'absolute', left: `-${previewSize + 60}px`, top: '50%', transform: 'translateY(-50%)', display: 'flex', alignItems: 'center' }}>
          <FieldPreview
        ...
        <div style={{ position: 'absolute', right: `-${previewSize + 60}px`, top: '50%', transform: 'translateY(-50%)', display: 'flex', alignItems: 'center' }}>
          <FieldPreview
      ...
      {!gameOver && (
        ...
        >
          ...
          <span style={{
            ...
            {currentPlayer === 1 ? 'Kék' : (gameMode === 'ai' ? 'AI (Zöld)' : 'Zöld')}
          </span>
        </motion.div>
      )}
    </div>
